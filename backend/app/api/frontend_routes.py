...
INGREDIENT_CATEGORIES = [
    "Tất cả", "Rau củ", "Thịt & Gia cầm", "Hải sản", "Trứng & Sữa",
    "Mì & Ngũ cốc", "Gia vị & Nêm", "Trái cây", "Bánh mì & Bánh ngọt",
    "Dầu ăn & Nước sốt", "Đồ hộp", "Các loại hạt",
]
RECIPE_CATEGORIES = [
    "Tất cả", "Cơm & Mì", "Canh & Súp", "Thịt", "Hải sản", "Rau",
    "Bánh & Bánh mì", "Tráng miệng", "Đồ uống", "Đồ ăn vặt",
]
TOKEN_ALIASES = {
    "chicken": {"ga"}, "beef": {"bo"}, "pork": {"heo", "lon"},
    "egg": {"trung"}, "rice": {"gao", "com"}, "fish": {"ca"},
    "shrimp": {"tom"}, "garlic": {"toi"}, "onion": {"hanh"},
    "tomato": {"ca", "chua"}, "carrot": {"ca", "rot"},
    "mushroom": {"nam"}, "potato": {"khoai", "tay"}, "cheese": {"pho", "mai"},
}
...
@router.post("/auth/register", response_model=FrontendAuthResponse, status_code=status.HTTP_201_CREATED)
def frontend_register(payload: FrontendAuthRequest, db: Session = Depends(get_db)):
    ...
    user = User(email=lookup, display_name=username, password_hash=hash_password(payload.password))
    db.add(user)
    db.commit()
    db.refresh(user)
    return FrontendAuthResponse(token=create_access_token(user.id), user=_frontend_user(user))
...
def frontend_login(payload: FrontendAuthRequest, db: Session = Depends(get_db)):
    user = db.scalar(select(User).where(User.email == payload.username.strip().casefold()))
