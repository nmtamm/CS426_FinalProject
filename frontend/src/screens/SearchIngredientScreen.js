...
export default function SearchIngredientScreen({ onNavigateToFindRecipes }) {
  const [searchQuery, setSearchQuery] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("Tất cả");
  const [isCategoryModalOpen, setIsCategoryModalOpen] = useState(false);
  const [selectedIngredients, setSelectedIngredients] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const [pageDirection, setPageDirection] = useState(1); // 1 = next (slide from right), -1 = prev (slide from left)

  // Filter Ingredients by Name and Category
  ...
  const handleFindRecipes = (ingredients) => {
    if (onNavigateToFindRecipes) {
      onNavigateToFindRecipes(ingredients);
    } else {
      const names = ingredients.map((i) => i.name).join(", ");
      Alert.alert("Tìm món ăn", `Đang tìm các công thức nấu ăn với: ${names}`);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Top Header & Search Area with Screen Margins */}
        ...
        >
          ...
          >
            ...
            {/* Paginated Results List Container */}
        ...
        {/* Dynamic Selected Ingredients Bottom Dock */}
